...
def get_top_eur_symbols(client, limit=10):
    try:
        ...
        eur_tickers = [t for t in tickers if t['symbol'] in valid and t['symbol'].endswith('EUR')]
...
def calculate_moving_averages(prices):
    ...
    return {"MA7": ma(prices, 7), "MA25": ma(prices, 25), "MA99": ma(prices, 99)}
...
def send_daily_report(trade_history, portfolio_value, baseline_value, daily_stats):
    if not SENDGRID_API_KEY or not EMAIL_RECEIVER:
...
- Only high-confidence trades (confidence >= 9)
...
- Growth needed: {growth_needed:.1f}%
- Win rate: {f"{win_rate*100:.1f}%" if win_rate else "N/A"} ({total_trades} trades)
- Strategy: {strategy} — {strategy_reason}
- Consecutive HOLDs this session: {consecutive_holds}

PORTFOLIO (Available EUR: €{eur_available:.2f}):
{json.dumps(portfolio_summary, indent=2)}

SELLABLE POSITIONS (can be sold):
...
- SHORT/MARGIN_BUY: use any BUYABLE SYMBOL, does NOT require EUR
- Always use full symbol names ending in EUR
...
[
  ...
  {{"action": "SHORT", "symbol": "DOGEEUR", "amount_eur": 30.00, "stop_loss_pct": 2.0, "reason": "...", "confidence": 8}}
]"""
    ...
    while True:
        try:
            ...
            if not portfolio_baseline or "value" not in portfolio_baseline:
                portfolio_baseline = {"value": portfolio_value, "time": str(datetime.now())}
            ...
            if consecutive_holds > 0:
                print(f"⏸️ Consecutive HOLDs this session: {consecutive_holds}/{MAX_CONSECUTIVE_HOLDS}")
            ...
            # Track consecutive HOLDs in session only
            if all(d.get("action") == "HOLD" for d in decisions):
                consecutive_holds += 1
            else:
                consecutive_holds = 0
            ...
            if datetime.now().hour == 23 and today != last_report_date:
                ...
                save_to_github(DAILY_STATS_FILE, daily_stats)
                errors_today = 0
