    ...
    for coin, amount in portfolio.items():
        if coin == "EUR":
            ...
            if amount >= MIN_TRADE_EUR:
                tradeable_positions.append(f"EUR (€{amount:.2f})")
        else:
            symbol = f"{coin}EUR"
            if symbol in market_data:
                ...
                if value >= MIN_TRADE_EUR:
                    tradeable_positions.append(f"{coin} (€{value:.2f}) → symbol: {symbol}")
...
{chr(10).join(tradeable_positions) if tradeable_positions else 'None'}
...
- ALWAYS use the exact symbol shown next to each position (e.g. ETHEUR, BTCEUR) — never just ETH or BTC
